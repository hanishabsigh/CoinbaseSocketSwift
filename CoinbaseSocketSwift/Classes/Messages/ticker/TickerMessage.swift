...
open class TickerMessage: ProductSequenceTimeMessage {
    ...
    public required init(json: [String: Any]) throws {
        ...
        let side = OrderSide(rawValue: json["side"] as? String ?? "")
        
        let lastSize = Double(json["last_size"] as? String ?? "")
        
        guard let bestBid = Double(json["best_bid"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("best_bid")
        }
        
        guard let bestAsk = Double(json["best_ask"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("best_ask")
        }
        
        guard let open24h = Double(json["open_24h"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("open_24h")
        }
        
        guard let high24h = Double(json["high_24h"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("high_24h")
        }
        
        guard let low24h = Double(json["low_24h"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("low_24h")
        }
        
        guard let volume24h = Double(json["volume_24h"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("volume_24h")
        }
        
        guard let volume30d = Double(json["volume_30d"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("volume_30d")
        }
