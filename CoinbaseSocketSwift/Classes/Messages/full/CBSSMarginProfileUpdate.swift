...
open class CBSSMarginProfileUpdate: CBSSProductMessage {
    ...
    public required init(json: [String: Any]) throws {
        ...
        guard let timestamp = (json["timestamp"] as? String)?.dateFromISO8601 else {
            throw CBSSError.responseParsingFailure("timestamp")
        }
        
        guard let userId = json["user_id"] as? String else {
            throw CBSSError.responseParsingFailure("user_id")
        }
        
        guard let profileId = json["profile_id"] as? String else {
            throw CBSSError.responseParsingFailure("profile_id")
        }
        
        guard let nonce = json["nonce"] as? Int else {
            throw CBSSError.responseParsingFailure("nonce")
        }
        
        guard let position = json["position"] as? String else {
            throw CBSSError.responseParsingFailure("position")
        }
        
        guard let positionSize = Double(json["position_size"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("position_size")
        }
        
        guard let positionCompliment = Double(json["position_compliment"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("position_compliment")
        }
        
        guard let positionMaxSize = Double(json["position_max_size"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("position_max_size")
        }
        
        guard let callSide = CBSSSide(rawValue: json["call_side"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("call_side")
        }
        
        guard let callPrice = Double(json["call_price"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("call_price")
        }
        
        guard let callSize = Double(json["call_size"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("call_size")
        }
        
        guard let callFunds = Double(json["call_funds"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("call_funds")
        }
        
        guard let covered = json["covered"] as? Bool else {
            throw CBSSError.responseParsingFailure("covered")
        }
        
        guard let nextExpireTime = (json["next_expire_time"] as? String)?.dateFromISO8601 else {
            throw CBSSError.responseParsingFailure("next_expire_time")
        }
        
        guard let baseFunding = Double(json["base_funding"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("base_funding")
        }
        
        guard let quoteBalance = Double(json["quote_balance"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("quote_balance")
        }
        
        guard let quoteFunding = Double(json["quote_funding"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("quote_funding")
        }
        
        guard let privateUpdate = json["private"] as? Bool else {
            throw CBSSError.responseParsingFailure("private")
        }
