//
//  MarginProfileUpdateMessage.swift
//  CoinbaseSocketSwift
//
...
open class MarginProfileUpdateMessage: ProductMessage {
    ...
    public required init(json: [String: Any]) throws {
        ...
        guard let nonce = json["nonce"] as? Int else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("nonce")
        }
        
        guard let position = json["position"] as? String else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("position")
        }
        
        guard let positionSize = Double(json["position_size"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("position_size")
        }
        
        guard let positionCompliment = Double(json["position_compliment"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("position_compliment")
        }
        
        guard let positionMaxSize = Double(json["position_max_size"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("position_max_size")
        }
        
        guard let callSide = OrderSide(rawValue: json["call_side"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("call_side")
        }
        
        guard let callPrice = Double(json["call_price"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("call_price")
        }
        
        guard let callSize = Double(json["call_size"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("call_size")
        }
        
        guard let callFunds = Double(json["call_funds"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("call_funds")
        }
        
        guard let covered = json["covered"] as? Bool else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("covered")
        }
        
        guard let nextExpireTime = (json["next_expire_time"] as? String)?.dateFromISO8601 else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("next_expire_time")
        }
        
        guard let baseFunding = Double(json["base_funding"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("base_funding")
        }
        
        guard let quoteBalance = Double(json["quote_balance"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("quote_balance")
        }
        
        guard let quoteFunding = Double(json["quote_funding"] as? String ?? "") else {
            throw CoinbaseSocketSwiftError.responseParsingFailure("quote_funding")
        }
