...
public class CBSSMatch: CBSSProductSequenceTimeMessage {
    ...
    public required init(json: [String: Any]) throws {
        ...
        guard let tradeId = json["trade_id"] as? Int else {
            throw CBSSError.responseParsingFailure("trade_id")
        }
        
        guard let makerOrderId = json["maker_order_id"] as? String else {
            throw CBSSError.responseParsingFailure("maker_order_id")
        }
        
        guard let takerOrderId = json["taker_order_id"] as? String else {
            throw CBSSError.responseParsingFailure("taker_order_id")
        }
