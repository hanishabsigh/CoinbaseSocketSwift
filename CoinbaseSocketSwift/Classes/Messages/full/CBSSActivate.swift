...
open class CBSSActivate: CBSSProductMessage {
    ...
    public required init(json: [String: Any]) throws {
        ...
        guard let stopType = json["stop_type"] as? String else {
            throw CBSSError.responseParsingFailure("stop_type")
        }
        ...
        guard let stopPrice = Double(json["stop_price"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("stop_price")
        }
        
        guard let size = Double(json["size"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("size")
        }
        
        guard let funds = Double(json["funds"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("funds")
        }
    
        
        guard let takerFeeRate = Double(json["taker_fee_rate"] as? String ?? "") else {
            throw CBSSError.responseParsingFailure("taker_fee_rate")
        }
